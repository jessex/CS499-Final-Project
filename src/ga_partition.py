...
ELITISM = 0.07 #percentage of population to definitely carry over to next gen
...
if __name__ == "__main__":
	#get and process params for run
	if len(argv) < 2:
		exit("Error: Please provide a file to read schedules from")
	global SCHEDULES, BEST_SO_FAR
	SCHEDULES = read_file(argv[1], 48) #get list of schedules from file
	input_invalid = True
	while input_invalid:
		try:
			pop_size = int(raw_input("Population size: "))
			if pop_size <= 1:
				print "Please provide only integer input >= 2 for population size"
			else:
				input_invalid = False
		except ValueError:
			print "Please provide only integer input >= 2 for population size"
	...
	while input_invalid:
		try:
			subset_size = int(raw_input("Subset size: "))
			if subset_size <= 1:
				print "Please provide only integer input >= 2 for subset size"
			else:
				input_invalid = False
		except ValueError:
			print "Please provide only integer input >= 2 for subset size"	
